...
- **One query, one tool**: uses only data and formulas generated by the code. The success rate has a dedicated formatter; other tools use their own deterministic `summary`.
- **Multiple tools, or an error**: deterministically lists the `summary`s of the executed tools and any errors.
...
| 7 | `failure_analysis` | In-depth analysis of failures: bursts, daily peaks, dominant cause | "Why does head H29 fail more often?" |
...
1. **Routing**: `router.route(question)` attempts to contact the LLM (if available). It constructs the system prompt from `tools.py`, sends the question and prompt, attempts to parse the response as valid JSON (with a retry if necessary), and normalizes any enum parameters. If all of this fails, it falls back to `fallback.keyword_route()`.
...
3. **Composition**: `composer.compose()` takes the results (including any errors) and produces a deterministic output. The availability of the LLM affects the routing, not the formulas or numbers displayed.
