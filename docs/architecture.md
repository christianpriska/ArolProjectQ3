...
**Streaming vs. in-memory ingestion.** `pipeline.ingest_dataset()` still processes
files one at a time (as above) either way, but has two output modes: the default
(`streaming=False`, used by library callers and the test suite) accumulates every
file's closure events in memory and concatenates + writes them once at the end,
returning the full `closure_events` DataFrame to the caller. `streaming=True`
(what the ingestion **CLI now uses by default**, `ingestion/__main__.py`) instead
writes each file's cleaned events straight to `closure_events.parquet` via an
incrementally-appended `pyarrow.parquet.ParquetWriter` (zstd-compressed) as soon as
they're ready, never holding the full 55M-row table in memory at once — the
function then returns `closure_events` as an **empty** DataFrame plus a
`closure_event_count` integer, and callers needing the actual rows read the
written Parquet file instead. This was added after the original always-in-memory
approach caused memory exhaustion during a full 89-file run.
...
- `idle_periods.parquet` — **3,493 periods** totaling **1,418.35 hours** after sampling gaps are excluded from known idle time.
...
| `GAP_FACTOR` | 2.0 | A sampling gap is flagged when the interval between two consecutive rows exceeds `GAP_FACTOR × median_interval` for that stretch — a standard multiple-of-median approach to temporal anomaly detection that adapts to each file's own sampling rate rather than a fixed-seconds cutoff. Used for per-file gap detection (`quality.compute_file_quality`), tagging a closure jump `data_quality="gap"` (`closures.detect_closures`), and splitting idle runs so unobserved time is not counted as known downtime (`idle._find_runs`). |
...
- **Near-perfect success rate**: 1,096 failures out of 55,130,461 observed closure
  ...
  success rate excluding no-load is **99.9965%**, not a rounded 100.00%.
  `_common.format_percentage()` switches to 4 decimal places for rates within 0.01
  of either boundary specifically so a result like this isn't misreported as a
  perfect 100.00% — a real bug an earlier version of the display code had.
